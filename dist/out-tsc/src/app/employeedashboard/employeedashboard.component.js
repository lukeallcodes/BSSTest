...
let EmployeedashboardComponent = class EmployeedashboardComponent {
    constructor(route, clientService, userService) {
        this.route = route;
    ...
    ngOnInit() {
        this.route.paramMap.pipe(switchMap(params => {
            ...
            this.client = client;
            this.assignedLocations = client.location.filter(location => this.user?.assignedlocations.includes(location._id) || location.zone.some(zone => this.user?.assignedzones.includes(zone._id)));
    ...
    startScan() {
        ...
        html5QrCode.start({ facingMode: "environment" }, { fps: 60, qrbox: 250 }, async (decodedText) => {
            const zoneId = decodedText;
            if (!this.client || !this.client.location)
                return;
    ...
    determineZoneOperation(zone, currentIsoTime, fullName, currentTime) {
    ...
    performCheckIn(zone, currentIsoTime, fullName, currentTime) {
        this.clearPreviousSession(zone);
    ...
    clearPreviousSession(zone) {
        zone.lastcheckin = '';
        zone.lastcheckout = '';
        zone.timespent = '';
    }
    performCheckOut(zone, currentIsoTime, fullName, currentTime) {
    ...
    updateScanMessage(zoneName, action, time) {
...
__decorate([
    ViewChild('qrReader')
], EmployeedashboardComponent.prototype, "qrReaderElement", void 0);
