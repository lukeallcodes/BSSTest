...
export class ManagerdashboardComponent implements OnInit {
  ...
  selectedZoneId: string | undefined; // Track the currently selected zone ID
  assignedUsersForZone: NewUser[] = [];
  ...
  unassignUserFromZone(userId: string, zone: Zone): void {
    this.updateZoneUserAssignments([userId], zone, false);
  }
  

  getUsersForLocation(locationId: string): NewUser[] {
  ...
  loadAssignedUsersForZone(zoneId: string): void {
    // Filter users who are assigned to the selected zone
    this.assignedUsersForZone = this.users.filter((user) =>
      user.assignedzones?.includes(zoneId)
    );
  }
  ...
  submitAssignedUsers(selectElement: HTMLSelectElement, location: Location): void {
    const selectedUserIds: string[] = Array.from(selectElement.selectedOptions).map(
      (option) => option.value
    );
    this.updateUserAssignments(selectedUserIds, location);
  }

  unassignUser(userId: string, location: Location): void {
  ...
  assignUsersToZone(selectElement: HTMLSelectElement, zone: Zone): void {
    const selectedUserIds: string[] = Array.from(selectElement.selectedOptions).map(
      (option) => option.value
    );
  ...
  private updateUserAssignments(userIds: string[], location: Location, assign: boolean = true): void {
    ...
    if (this.client && this.client._id) {
      this.clientService.updateClient(this.client._id, this.client).subscribe({
        next: () => this.loadClientAndUsers(), // Reload to reflect changes
  ...
  private updateZoneUserAssignments(
    userIds: string[],
    zone: Zone,
    assign: boolean = true
  ): void {
  ...
  getUnassignedUsersForZone(zoneId: string): NewUser[] {
    ...
    return this.users.filter((user) =>
      user.assignedlocations?.includes(zoneLocation._id) &&
